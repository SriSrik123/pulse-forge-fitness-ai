...
Generated by PulseTrack AI
  ...
  return (
    <div className="space-y-6 animate-fade-in bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen p-4">
      <div className="text-center py-6 bg-white rounded-xl shadow-sm border">
        ...
        <div className="flex gap-2 justify-center flex-wrap">
          <Button variant="outline" size="sm" onClick={downloadWorkout} className="bg-white border-gray-200 hover:bg-gray-50">
            <Download className="h-4 w-4 mr-2" />
            Download
          </Button>
          <Button variant="outline" size="sm" onClick={printWorkout} className="bg-white border-gray-200 hover:bg-gray-50">
        ...
        <Card className="bg-blue-50 border-blue-200 shadow-sm">
          <CardContent className="pt-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-blue-900 font-medium">{contextText}</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="bg-white border-gray-200 shadow-sm">
        ...
        <CardContent className="space-y-4 pt-4">
          {workout.exercises.map((exercise, index) => (
            <div key={index} className="p-4 bg-blue-50 rounded-lg border border-blue-100 space-y-3">
              {workout.sport === 'swimming' ? (
                <div className="text-center space-y-3">
                  <div className="bg-blue-100 rounded-lg p-3 border border-blue-200">
                    <h4 className="font-bold text-xl text-blue-900 mb-2">{exercise.name}</h4>
                    <div className="text-lg font-semibold text-blue-800">{exercise.reps}</div>
                    {exercise.rest && (
                      <div className="text-sm text-blue-700 mt-1">Rest: {exercise.rest}</div>
                    )}
                  </div>
                  {exercise.description && (
                    <div className="bg-white p-3 rounded border border-gray-200">
                      <p className="text-sm text-gray-700 italic">{exercise.description}</p>
                    </div>
                  )}
                </div>
              ) : (
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      {exercise.rest && (
                        <div className="col-span-2">
                          ...
                          <span className="ml-2 text-gray-800">{exercise.rest}</span>
                        </div>
                      )}
                    </div>
                  </div>
