...
export function WorkoutViewer({ workoutType, workoutId, generatedWorkoutData }: WorkoutViewerProps = {}) {
  ...
  const loadScheduledWorkouts = async () => {
    ...
    try {
      ...
      // Get scheduled workouts for the selected date that are not completed
  ...
  const handleWorkoutComplete = async () => {
    ...
    // Mark the scheduled workout as completed if it exists
    if (currentScheduledWorkoutId) {
      try {
        await supabase
          .from('scheduled_workouts')
        ...
        console.warn('Failed to update scheduled workout:', error)
      }
    }
    
    // Clear current workout and refresh today's workouts
    setWorkout(null)
    setCurrentWorkoutId(null)
    setCurrentScheduledWorkoutId(null)
    
    // Reload scheduled workouts to remove completed ones
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <div className="mb-4">
          ...
          <p className="text-lg font-medium text-foreground">Nothing left to do today! 🎉</p>
          <p className="text-sm text-muted-foreground mt-2">You've completed all your scheduled workouts for {format(selectedDate, 'MMMM d, yyyy')}.</p>
          <p className="text-sm text-muted-foreground">Go to Training Manager to create more workouts.</p>
        </div>
                  ...
                  {exercise.description && (
                    <div className="glass border-0 p-3 rounded border">
                      <p className="text-sm text-muted-foreground italic">
                        {typeof exercise.description === 'object' && exercise.description
                          ? (exercise.description as any).description || JSON.stringify(exercise.description)
                          : exercise.description
                        }
                      </p>
                    </div>
